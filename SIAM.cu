...
double imSOCSigmafc(double om, void *params)
{
  ...
  return linear_interpl(om,p->omega,p->Y,p->N);
}
...
void SIAM::get_Ps()
{
  ...
  for (int i = 0; i < GPU_N; i++)
  {
	    ...
	    cudaSetDevice(i);
			...
			cudaMemcpyAsync(plan[i].d_omega, r->omega, N * sizeof(double), cudaMemcpyHostToDevice, plan[i].stream);
      
			//Copy Ap,Am from CPU
			cudaMemcpyAsync(plan[i].d_Ap, r->Ap, N * sizeof(double), cudaMemcpyHostToDevice, plan[i].stream);
			cudaMemcpyAsync(plan[i].d_Am, r->Am, N * sizeof(double), cudaMemcpyHostToDevice, plan[i].stream);
  ...
  for (int i = 0; i < GPU_N; i++)
  {
      ...
      {
          r->P1[plan[i].i0+j] = plan[i].h_P1[j];
          r->P2[plan[i].i0+j] = plan[i].h_P2[j];
      }
...
void SIAM::get_SOCSigma(){
  ...
  for (int i = 0; i < GPU_N; i++)
  {
      ...
      cudaMemcpyAsync(plan[i].d_Ap, r->Ap, N * sizeof(double), cudaMemcpyHostToDevice, plan[i].stream);
      cudaMemcpyAsync(plan[i].d_Am, r->Am, N * sizeof(double), cudaMemcpyHostToDevice, plan[i].stream);
      cudaMemcpyAsync(plan[i].d_P1, r->P1, N * sizeof(double), cudaMemcpyHostToDevice, plan[i].stream);
      cudaMemcpyAsync(plan[i].d_P2, r->P2, N * sizeof(double), cudaMemcpyHostToDevice, plan[i].stream);
  ...
  for (int i=1; i<N-1; i++)
  { 
    const double a = r->omega[0], b = r->omega[N-1]; // limits of integration
    ...
    r->SOCSigma[i] = complex<double>(result/M_PI,imSOCSigma[i]);
  }
  
  r->SOCSigma[0] = r->SOCSigma[1];
  r->SOCSigma[N-1] = r->SOCSigma[N-2];
...
void SIAM::get_Ps()
{
  ...
  std::copy(r->omega,(r->omega)+N,omega32);
  std::copy(r->Ap,(r->Ap)+N,Ap32);
  std::copy(r->Am,(r->Am)+N,Am32);
  ...
  for (int i = 0; i < GPU_N; i++)
  {
      ...
      for (int j = 0; j < plan[i].N_LOCAL; j++)
      {
          r->P1[plan[i].i0+j] = (double) plan[i].h_P1[j];
          r->P2[plan[i].i0+j] = (double) plan[i].h_P2[j];
      }
...
void SIAM::get_SOCSigma(){
  ...
  std::copy(r->Ap,(r->Ap)+N,Ap32);
  std::copy(r->Am,(r->Am)+N,Am32);
  std::copy(r->P1,(r->P1)+N,P132);
  std::copy(r->P2,(r->P2)+N,P232);
